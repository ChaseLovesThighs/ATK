...
function ATK:CreateATK(ATKName)
	...
	closeLib.MouseButton1Click:Connect(function()
		...
		if isClosed then
			...
			(game.TweenService:Create(closeLib, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
				Rotation = 360
			})):Play();
			MainWhiteFrame:TweenSize(UDim2.new(0, 424, 0, 58), "In", "Linear", 0.12);
			(game.TweenService:Create(MainWhiteFrame_2, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
				BackgroundTransparency = 1
			})):Play();
			(game.TweenService:Create(MainWhiteFrame, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
				BackgroundTransparency = 1
			})):Play();
		else
			closeLib.Image = "rbxassetid://4988112250";
		...
		function ElementHandler:Dropdown(dInfo, list, callback)
			...
			ImageButton.MouseButton1Click:Connect(function()
				if isDropped then
					...
					(game.TweenService:Create(ImageButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
						Rotation = 0
					})):Play();
					wait(0.1);
					UpdateSize();
